...
function buildFormTemplate(conf: any, child: any, type: any) {
  let labelPosition = ''
...
function buildFromBtns(conf: any, type: any) {
  let str = ''
...
// span不为24的用el-col包裹
function colWrapper(element: any, str: any) {
...
const layouts = {
  ...
  rowFormItem(element: any) {
    const type = element.type === 'default' ? '' : `type="${element.type}"`
    const justify = element.type === 'default' ? '' : `justify="${element.justify}"`
    const align = element.type === 'default' ? '' : `align="${element.align}"`
    const gutter = element.gutter ? `gutter="${element.gutter}"` : ''
    const children = element.children.map((el: any) => layouts[el.layout](el))
...
// el-buttin 子级
function buildElButtonChild(conf: any) {
  const children = []
  if (conf.default) {
...
// el-input innerHTML
function buildElInputChild(conf: any) {
  const children = []
  if (conf.prepend) {
...
function buildElSelectChild(conf: any) {
  const children = []
...
function buildElRadioGroupChild(conf: any) {
  const children = []
  if (conf.options && conf.options.length) {
...
function buildElCheckboxGroupChild(conf: any) {
  const children = []
  if (conf.options && conf.options.length) {
...
function buildElUploadChild(conf: any) {
  const list = []
...
export function makeUpHtml(conf: any, type: any) {
  const htmlList: any = []
  confGlobal = conf
  someSpanIsNot24 = conf.fields.some((item: any) => item.span !== 24)
