  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="字典名称" prop="dict_name">
        <el-select v-model="queryParams.dict_name" size="small">
          <el-option
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="字典类型">
          <el-input v-model="queryParams.dict_name" :disabled="true" />
        </el-form-item>
        ...
        <el-form-item label="备注" prop="remark">
          <el-input v-model="form.remark" autosize type="textarea" placeholder="请输入内容"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  created() {
    ...
    this.getType(dictId);
    this.getTypeList();
    // this.statusOptions = [{dict_value : "0",dict_label: "启用"},{dict_value : "1",dict_label: "停用"}];
  ...
  methods: {
    ...
    getType(dictId) {
      getType(dictId).then(response => {
        ...
        this.queryParams.dict_type = dictId;
        this.queryParams.dict_name = response.dict_name;
    ...
    getTypeList() {
      listType().then(response => {
        console.log(response);
        this.typeOptions = response.docs;
      });
    },
    /** 查询字典数据列表 */
    getList() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const dictid = row._id || this.ids;
      this.$confirm('是否确认删除字典编码为"' + row.dict_code + '"的数据项?', "警告", {
