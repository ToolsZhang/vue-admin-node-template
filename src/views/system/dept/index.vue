<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="is_deal_appeal" align="center" label="是否诉求处理部门" :formatter="isDealAppealFormat" width="300"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="诉求处理">
              <el-radio-group v-model="form.is_deal_appeal">
  ...
  methods: {
    ...
    getList() {
      ...
      listDept(this.queryParams).then(response => {
        this.deptList = this.handleTree(response, "_id","parent");
        this.loading = false;
      });
    },
    /** 转换部门数据结构 */
    ...
    isDealAppealFormat(row, column) {
      return row.is_deal_appeal === '0' ? '是' : '否';
    },
    // 取消按钮
    cancel() {
      this.open = false;
    ...
    handleAdd(row) {
      ...
      listDept({}).then(response => {
        this.deptOptions = [];
        let dept = { _id: 0, dept_name: '于田诉求政务平台', children: [] };
        dept.children = this.handleTree(response, "_id","parent");
    ...
    handleUpdate(row) {
      ...
      listDeptExcludeChild(row._id).then(response => {
        this.deptOptions = [];
        let dept = { _id: 0, dept_name: '于田诉求政务平台', children: [] };
        dept.children = this.handleTree(response, "_id","parent");
        this.deptOptions.push(dept);
