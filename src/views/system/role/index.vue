<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色id" prop="_id" width="220" />
      <el-table-column label="角色名称" prop="role_name" :show-overflow-tooltip="true" width="150" />
      <el-table-column label="权限字符" prop="role_key" :show-overflow-tooltip="true" width="100" />
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm('确认要"' + text + '""' + row.role_name + '"角色吗?', "警告", {
          ...
          return changeRoleStatus(row._id, row.status);
        }).then(() => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const roleIds = row._id || this.ids;
      this.$confirm('是否确认删除角色id为"' + roleIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
