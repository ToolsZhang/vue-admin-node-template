...
<el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
        ...
        <el-table v-loading="loading" :data="appealList" @selection-change="handleSelectionChange" @row-click="appealDetail">
          ...
          <el-table-column label="申请人" align="center" key="masses" :formatter="massesFormat" v-if="columns[1].visible" width="330"/>
          <el-table-column label="债务金额" align="center" key="money" prop="money"  v-if="columns[2].visible" width="200"/>
          <el-table-column label="家庭住址" align="center" key="address" :formatter="addressFormat" v-if="columns[3].visible"/>
          <el-table-column label="已还金额" align="center" prop="repay" v-if="columns[4].visible"  width="200">
            <template slot-scope="scope">
              <span>{{  scope.row.repay ? scope.row.repay : '0' }}</span>
            </template>
          </el-table-column>
          <el-table-column label="诉求状态" align="center" prop="state" :formatter="stateFormat" v-if="columns[5].visible" width="120" />
                  ...
                  <el-timeline>
                    <el-timeline-item :type="'primary'" :size="'large'" :icon="process.icon" :timestamp="dateFormatISO(process.createdAt,fmt = 'YYYY/mm/dd')" placement="top" v-for="(process, index) in appealProcessList" :key="index">
                      <el-card>
                        ...
                        <p v-if="process.repay && process.repay !== ''">
                          已偿还金额 : {{ process.repay }}
                        </p>
    ...
    <el-dialog :title="'诉求完结'" :visible.sync="finishOpen" width="600px" append-to-body>
      <el-form ref="finishForm" :model="finishForm" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="已偿还金额" prop="repay">
              <el-input-number v-model="finishForm.repay" controls-position="right" :min="0" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-dialog :title="'诉求部分完结'" :visible.sync="partOpen" width="600px" append-to-body>
      <el-form ref="partForm" :model="partForm" :rules="rules" label-width="120px">
        <el-row>
          <el-col :span="24">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="已偿还金额" prop="repay">
              <!-- <el-input v-model="partForm.repay" type="number" placeholder="请输入偿还金额" ></el-input> -->
              <el-input-number v-model="partForm.repay" controls-position="right" :min="0" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="归属部门" >
              <el-select v-model="handleForm.handleDept" placeholder="暂无" >
                <el-option
...
<script>
import { handleListDebtAppeal, listAppealProcess, getAppeal, updateAppeal, delAppeal, exportAppeal, changeAppealStatus, handleAppealById, superiorAppeal, partAppeal, finishAppeal } from "@/api/appeal/appeal";
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 2, label: `债务金额`, visible: true },
        { key: 3, label: `诉求人住址`, visible: true },
        { key: 4, label: `已偿还`, visible: true },
        { key: 5, label: `诉求状态`, visible: true },
        { key: 6, label: `创建时间`, visible: true },
      ],
      // 表单校验
      rules: {
        repay: [
          { required: true, message: "已偿还金额不能为空", trigger: "blur" }
        ],
        solution: [
  ...
  methods: {
    ...
    getList(param) {
      ...
      getDeptIdByUser().then(response => {
        ...
        handleListDebtAppeal(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
